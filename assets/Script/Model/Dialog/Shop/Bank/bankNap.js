...
cc.Class({
	...
	onClickNap: function(){
		if (!this.labelNumber.string.length) {
			cc.RedT.inGame.notice.show({title:"NẠP RED", text: "Vui lòng chọn ngân hàng muốn nạp."});
		}else if (this.hinhThuc === '1' && this.inputKhop.string.length < 6) {
			cc.RedT.inGame.notice.show({title:"NẠP RED", text: "Nhập mã giao dịch ngân hàng..."});
		}else if (this.hinhThuc === '2' && this.inputSTK.string.length < 6) {
			cc.RedT.inGame.notice.show({title:"NẠP RED", text: "Vui lòng nhập chính xác số tài khoản của bạn."});
		}else if (this.hinhThuc === '3' && this.inputNameGo.string.length < 6) {
			cc.RedT.inGame.notice.show({title:"NẠP RED", text: "Vui lòng nhập Họ Tên người đi chuyển tiền."});
		}else if (helper.getOnlyNumberInString(this.inputTien.string)>>0 < 200000) {
			cc.RedT.inGame.notice.show({title:"NẠP RED", text: "Nạp tối thiểu 200.000, tối đa 1.000.000.000"});
